...
def train_multinomial(total_bag,ham,spam,df):
    prior_prob={}
    tct_ham={}
    tct_spam={}
    cond_prob_ham={}
...
def apply_multinomial(prior_prob_ham,prior_prob_spam,cond_prob_ham,cond_prob_spam,vocab,d):
    temp_wordlist=[]
    words=d.split()
    for i in words:
        if i in vocab:
...
if __name__ == "__main__":
    ...
    prior_prob_ham,prior_prob_spam,cond_prob_ham,cond_prob_spam=train_multinomial(total_bag,train_ham,train_spam,df_train)
    ...
    y_pred=np.array([])
    for d in test_ham:
        if apply_multinomial(prior_prob_ham,prior_prob_spam,cond_prob_ham,cond_prob_spam,total_bag,d)==1:
            y_pred=np.append(y_pred,[1])
        else:
            y_pred=np.append(y_pred,[0])
    for d in test_spam:
        if apply_multinomial(prior_prob_ham,prior_prob_spam,cond_prob_ham,cond_prob_spam,total_bag,d)==0:
            y_pred=np.append(y_pred,[0])
        else:
            y_pred=np.append(y_pred,[1])
    ...
    for i in range(len(y_pred)):
        if y_pred[i]==y_test[i]:
            ...
            failed=failed+1
    print("Bag of Words Model Training Dataset-----------------------------------------",df_train)
    print("Accuracy for Bag of Words Model using Naive Bayes= {:.4f}".format((passed)/(passed+failed)))
    print("F1 score for Bag of Words Model using Naive Bayes= {:.4f}".format(f1_score(y_test, y_pred, average="binary")))
    print("Precision score for Bag of Words Model using Naive Bayes= {:.4f}".format(precision_score(y_test, y_pred, average="binary")))
    print("Recall score for Bag of Words Model using Naive Bayes= {:.4f}".format(recall_score(y_test, y_pred, average="binary")))
